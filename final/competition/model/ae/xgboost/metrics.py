...
def compute_metrics(labels, scores, target_names=None, risk_list=[5, 2, 1, 0.5, 0.25]):
    ...
    with warnings.catch_warnings():
        ...
        for risk in risk_list:
            df[f"precis_{risk}% recall_{risk}%".split()] = pd.DataFrame(
                scores_v2(labels, scores, risk)
            )
    error = scores - labels
