    ...
    if return_preds:
        ...
        attn = [
            np.concatenate(  # append with zeros
                (
                    cur_attn,
                    np.zeros(
                        (cur_attn.shape[0], abs(cur_attn.shape[1] - 500))
                    ),
                ),
                1,
            )
        ...
        feat = [
            np.concatenate(
                (cur_feat, 
                 np.zeros(
                    (cur_feat.shape[0], abs(cur_feat.shape[1] - 500)))
                ), 1
            )
        ...
        return_tuple = return_tuple + ((ids_lst, prediction_scores.numpy(), order_labels, attn, feat),)
