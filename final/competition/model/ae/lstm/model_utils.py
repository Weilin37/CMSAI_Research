...
def build_lstm_dataset_v2(data_x_path, data_y_path, vocab_path, target_names):
    """
    Build a dataset from a given X, y and vocab paths.

    Arguments:
    ---------
        data_x_path (str) : path to X numpy file
        data_y_path (str): path to y numpy file
    ...
    """
    log("Load data and vocab")
    X = np.load(data_x_path)
    Y = np.load(data_y_path)
    X = X[:10000, :]  # TODO: Remove later
    Y = Y[:10000, :]
    vocab = torch.load(vocab_path)

    num_examples = X.shape[0]
    pids = range(num_examples)
    ...
    for pid in pids:
        ...
        label = Y[pid].tolist()
        data.append((pid, torch.tensor(label), torch.tensor(tokens)))
    labels = set(target_names)
...
def epoch_val_lstm(
    model, dataloader, criterion, return_preds=False, test=0, max_len=500
):
    """
    ...
    prediction_scores = torch.sigmoid(torch.Tensor(prediction_scores))
    ...
    if return_preds:
        ...
        attn = [
            np.concatenate(  # append with zeros
                (
                    cur_attn,
                    np.zeros((cur_attn.shape[0], abs(cur_attn.shape[1] - 500))),
                ),
                1,
            )
            if cur_attn.shape[1] != 500
            else cur_attn
            for cur_attn in attn
        ]
        ...
        feat = [
            np.concatenate(
                (cur_feat, np.zeros((cur_feat.shape[0], abs(cur_feat.shape[1] - 500)))),
                1,
            )
            if cur_feat.shape[1] != 500
            else cur_feat
        ...
        return_tuple = return_tuple + (
            (ids_lst, prediction_scores.numpy(), order_labels, attn, feat),
        )
...
def evaluate_explain(
    ...
    with torch.no_grad():
        for idx, (text, text_lengths, labels) in enumerate(iterator):
            ...
            # if text.size()[1] < 500:
            #    delta_text = abs(text.size()[1]-500)
            ...
            if cur_attn.size()[1] != max_length:
                ...
                attn[idx] = torch.cat(
                    (
                        cur_attn,
                        torch.zeros(
                            (cur_attn.size()[0], delta_text), dtype=cur_attn.dtype
                        ),
                    ),
                    1,
                ).to(device)
            ...
            if cur_feat.size()[1] != max_length:
                ...
                feat[idx] = torch.cat(
                    (
                        cur_feat,
                        torch.zeros(
                            (cur_feat.size()[0], delta_text), dtype=cur_feat.dtype
                        ),
                    ),
                    1,
                ).to(device)
...
def train_pos_neg(
    ...
    for idx, (
        (pos_text, pos_text_lengths, pos_labels),
        (neg_text, neg_text_lengths, neg_labels),
    ) in enumerate(zip(pos_iterator, neg_iterator)):
        ...
        if detach_hidden:
            if batch_size == 0:
                raise ValueError(
                    "Batch_size in training needs to number with detach_hidden"
                )
            hidden = model.init_hidden(batch_size)
            hidden = repackage_hidden(hidden)
        ...
        if metric == "acc":
            ...
            batch_metric = metrics.compute_single_metric(
                labels.cpu().numpy(), predictions.detach().cpu().numpy()
            )
...
def build_text_dataset(df, feat_colnames, label_colnames, vocab=None, min_freq=1):
    ...
    def create_tuple(labels, features, vocab):
        try:
            ...
            sentence = torch.tensor(
                [
                    vocab.stoi[x] if x in vocab.stoi.keys() else vocab.stoi["<unk>"]
                    for x in sentence
                ]
            )
    ...
    # create dataset
    data = df.apply(
        lambda row: create_tuple(row[label_colnames], row[feat_colnames], vocab), axis=1
    )
