...
def dump_results(pids, event_names, labels, scores, filepath):
    ...
    df = pd.concat([pd.DataFrame(pids, columns=['patient_id']), 
                    pd.DataFrame(labels.astype(int), columns=event_cols), 
                    pd.DataFrame(scores, columns=score_cols)], axis=1)
...
def load_results(filepath):
    ...
    return df['patient_id'].to_list(), event_names, df[event_cols].values, df[score_cols].values
...
def print_model(model):
    print(model)
    print('Total number of parameters', sum(map(lambda p:reduce(mul, p.shape), model.parameters())))
