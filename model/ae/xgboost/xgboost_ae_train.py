...
#                            }
    ...
    for num_features in NUM_FEATURES_LIST:
        for label in labels:
            start = time.time()
            print('Training for num_features={}, label={}...'.format(num_features, label))
            #Prepare the input train & validation data path
            s3_input_train = sagemaker.inputs.TrainingInput(s3_data='s3://{}/{}/{}/{}/train'.format(BUCKET, DATA_PREFIX, num_features, label), content_type='csv')
            s3_input_validation = sagemaker.inputs.TrainingInput(s3_data='s3://{}/{}/{}/{}/val'.format(BUCKET, DATA_PREFIX, num_features, label), content_type='csv')
            ...
            xgb = sagemaker.estimator.Estimator(container,
                                                ...
                                                output_path='s3://{}/{}/{}/{}/output'.format(BUCKET, MODEL_PREFIX, num_features, label),
                                                sagemaker_session=sess)

            xgb.set_hyperparameters(eval_metric=EVALUATION_METRIC,
                                    objective=OBJECTIVE,
            ...
            temp_result_path = os.path.join(TRAIN_TEMP_RESULTS_DIR, '{}_{}.txt'.format(num_features, label))
    ...
    df_results = pd.DataFrame(training_results, columns=['class', 'num_features', 'val_auc', 'best_model_path'])
