"""LSTM-based Model Architectures. Integrate back into lstm_models.py upon completion"""
...
class AttLSTM(SimpleLSTM):
    ...
    def forward(self, tokens, ret_attn=False):
        ...
        attn_weights = torch.bmm(output, out.unsqueeze(2)).squeeze(2) / ((tokens.shape[1]) ** 0.5)
        attn_weights = nn.functional.softmax(attn_weights + mask_feats, -1)
        
        context = torch.bmm(output.transpose(1, 2), attn_weights.unsqueeze(-1)).squeeze(-1)
        ...
        if ret_attn:
            return pred.detach().cpu().numpy(), attn_weights.detach().cpu().numpy()
    ...
    def forward_shap(self, token_ids, mask, full_id_matrix=False):
        ...
        #context = torch.bmm(att_weights.unsqueeze(1), output).squeeze(1)
        attn_weights = torch.bmm(output, out.unsqueeze(2)).squeeze(2) / (sum(mask) ** 0.5)
        
        soft_attn_weights = nn.functional.softmax(attn_weights, 1)
        
        context = torch.bmm(output.transpose(1, 2), soft_attn_weights.unsqueeze(-1)).squeeze(-1)
