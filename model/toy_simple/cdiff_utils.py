...
class AttNoHtLSTM(SimpleLSTM):
    ...
    def forward_shap(self, token_ids, mask, full_id_matrix=False):
        ...
        embedded0 = torch.matmul(token_ids, self.emb_layer.weight).unsqueeze(0)

        embedded = embedded0[:, : sum(mask), :]
        ...
        del concat_out, context, attn_weights, soft_attn_weights, out, output, hidden, embedded, embedded0, token_ids 
...
def get_similarity(scores, tokens, freedom):
    ...
    if n_gt > 0:
        ...
        sim = len(set(scores_idx).intersection(gt_idx)) / n_gt
        return sim
    return -1