...
class AttentionRNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim,
                 output_dim, bidirectional, dropout, padding_idx=None,
                 device=None):
        super().__init__()
    ...
    def attention_layer(self, output, final_hidden, explain=False, mask=None):
        '''Dot Attention'''
        stacked_hidden = (torch.cat((final_hidden[0], final_hidden[1]), dim=1)
        ...
        new_stacked_hidden = (torch.bmm(output.transpose(1, 2), soft_attn_weights)).squeeze(2)
        #print(f'New stacked hidden dimensions: {new_stacked_hidden.shape}')
        
    
        if self.bidirectional:
            new_hidden = torch.stack((new_stacked_hidden[:, :self.hidden_dim],
    ...
    def forward(self, text, text_lengths, hidden, explain=False):
        ...
        if explain:
            hidden, soft_attn_weights = self.attention_layer(output, hidden, explain=True, mask=mask_feats)
        else:
            hidden = self.attention_layer(output, hidden, mask=mask_feats)
        
        #print(f'Returned  hidden dimensions: {hidden.shape}')
        fhidden = (self.dropout(torch.cat((hidden[-2,:,:], hidden[-1,:,:]), dim = 1))
                  if self.bidirectional 
        ...
        return self.fc(fhidden), fhidden
